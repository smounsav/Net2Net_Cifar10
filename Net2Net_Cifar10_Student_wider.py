...
# Net2Net_Cifar10_Student_wider.py v.90 01.05.2017
#
...
#
# Net2Net_Cifar10_Student_wider.py trains a network containing an extension 
# of the first CNN layer of the teacher CNN.
...
#######In Student_wider extension 16 output channels --> 64
...
#######In Student_wider extension 16 input channels --> 64
#   - a relu activation layer
#   - a max_pool pooling layer with patch 2x2
#   - a normalisation layer
...
group.add_argument("--rand", help="Random initialisation", action="store_true")
group.add_argument("--nonoise", help="Do not add noise to replicated weights", action="store_true")
...
rand_flag = args.rand
nonoise_flag = args.nonoise

# Define log path
PATH_TENSORBOARD_LOG = './cifar-10-python/cifar-10-logs/student-wider'
string_lr = str(learning_rate)
string_lr = string_lr.replace('.', '')
PATH_TENSORBOARD_LOG = PATH_TENSORBOARD_LOG + '-lr' + string_lr
if rand_flag:
    PATH_TENSORBOARD_LOG = PATH_TENSORBOARD_LOG + '-rand'
if nonoise_flag:
    PATH_TENSORBOARD_LOG = PATH_TENSORBOARD_LOG + '-nonoise'
...
with tf.Graph().as_default() as student_graph:
    ...
    with tf.name_scope("ConvLayer1"):
        W_conv1 = tf.Variable(tf.zeros([5, 5, 3, 64])) # 5x5 patch, 3 input channels, 32 output channels
    ...
    with tf.name_scope("ConvLayer2"):
        W_conv2 = N2NMoTb.init_weight([5, 5, 64, 64]) # 5x5 patch, 32 input channels, 64 output channels
...
# Calculate new weights and biases with net2WidetNet method
new_W_conv1, new_b_conv1, new_W_conv2 = N2N.net2WiderNet_CNN(teacher_W_conv1, teacher_b_conv1, teacher_W_conv2, 48, random_init=rand_flag, add_noise=not(nonoise_flag))
new_W_conv1_tensor = tf.convert_to_tensor(new_W_conv1, dtype=tf.float32, name="weight")
new_b_conv1_tensor = tf.convert_to_tensor(new_b_conv1, dtype=tf.float32, name="bias")
new_W_conv2_tensor = tf.convert_to_tensor(new_W_conv2, dtype=tf.float32, name="weight")

# Transfer weights to student model
W_conv1_update  = W_conv1.assign(new_W_conv1_tensor)
sess.run(W_conv1_update)
b_conv1_update = b_conv1.assign(new_b_conv1_tensor)
sess.run(b_conv1_update)
W_conv2_update = W_conv2.assign(new_W_conv2_tensor)
...
for epoch in range(100001): 
    ...
    train_data_dict = {x: batch_data, y_:  batch_labels, keep_prob: 0.5, lr: learning_rate}
    acc_train_data_dict = {x: batch_data, y_:  batch_labels, keep_prob: 1, lr: learning_rate}
    test_data_dict = {x: test_dataset, y_: test_labels, keep_prob: 1, lr: learning_rate}
    if epoch % 100 == 0:
